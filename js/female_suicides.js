...
(function() {
    ...
    window.onload = function() {
        svgContainer = d3.select('.females')
    ...
    function makeGraph(csvData) {
        data = csvData;
        data = data.filter((row) => row["sex"] == "female");
    ...
    function makeLabels() {
        svgContainer.append('text')
          ...
          .text("Suicides for Females Over Time");
    ...
    function plotGraph(map) {
        ...
        let tip_container = div.append('svg')
          .attr('width', 300)
          .attr('height', 300);
        ...
        .on("mouseover", (d) => {
          makeScatterPlot(scatter_data, tip_container);
          div.transition()
            .duration(200)
            .style("opacity", .9)
